...
我们使用明确的时间值进行单元测试。你可以看下 [tests.ts](../test/unit/tween.test.ts) 这个例子，看看我们如何用不同的值调用`TWEEN.update()` 来模拟时间传递。
...
> **Note** 如果你使用 `TWEEN.update()` 作为默认控制所有补间的方式，则无需直接调用 `tween.update()`，但是我们建议你直接如上例所示 [创建自己的补间集](#控制补间集) 或手动更新补间。 使用组或单独控制的补间的概念很像避免在 JavaScript 代码中使用全局变量的做法：它可以防止一个组件意外破坏其他一些不相关组件的行为。
