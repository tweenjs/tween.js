...
library, then place the output in your project. [Bun](https://bun.com/docs/installation) is required for this.
...
You need to [install Bun](https://bun.com/docs/installation) first--it is a standalone runtime, so Node.js and `npm` are not needed. Then, cd to `tween.js`'s (or wherever you cloned the repo) directory and run:
...
features or fixes) in `test/unit/tween.test.ts` to be accepted. See
